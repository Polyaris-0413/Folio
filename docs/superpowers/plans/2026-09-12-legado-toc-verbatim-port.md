...
> **实施说明（已落地部分）**：阅读链路已按「`currentChapterIndex` 继续沿用 + `coerceIn` 兜底」接入，**尚未**实现 `getDurChapter` 的章名相似度对齐。原因是该对齐的输入（旧章名列表）在 Folio 侧没有留存——`Book` 只存章号与章内偏移，不存章名，而 legado 侧是拿「上次的目录」比对「新目录」。要做需先给 `Book` 增加章名快照列，属独立改动，故留待你决定是否需要（换规则属用户主动行为，coerceIn 的退化可接受）。

---

## 附丙：实施记录与已知差异（2026-09-12 实施后补）

**已验证**（证据随提交保留）：

- 逐字节一致性：`scripts/check-port-drift.sh` 对 18 个搬运文件零漂移（仅行尾差异，与仓库既有 mobi 移植同一语义）。
- Room schema：v8→v9→v10 两步迁移生成的 `10.json` 与仓库既有那份逐字节一致（含 `identityHash`），先前断裂的 schema 链补全。
- JVM 单测 10 项：规则资产结构、规则命中自身示例、UTF-8/GBK 编码探测可还原、BOM 判定、MD5 标准向量、字数格式阈值、`book.tocRule` 载体语义、`BookChapter` 相等性与文件名格式。
- 设备侧 instrumented 6 项（PJD110 实机）：规则打分择优、固定规则切章+前言章、无规则命中的字数兜底、超长章节拆分（含 `splitLongChapter=false` 分支）、**v8 库升级到 v10 后书籍数据完整保留且新增规则表**、模板自带测试。
- 真机冷启动无崩溃（全新安装）。
- 规则管理界面已在真机过目（设置页入口 / 规则列表 / 启用开关 / 编辑弹窗），并据此修掉两个问题：全新安装时规则页为空（缺启动播种）、启用开关点了没反应（见下条）。

**界面修复记录（2026-09-12）**：开关失灵不是点击没送达——日志证实 `onCheckedChange` 触发且写入落库，但界面渲染探针在点击后再未触发。根因是移植来的 `TxtTocRule` 重写 `equals`/`hashCode` 只比较 `id`（legado 原样，用于其列表去重），而 Compose 的 `collectAsState` 按结构相等判定状态变化，于是「只有 enable 变了」的新列表被判为与旧列表相等、更新被丢弃。修复落在 Folio 侧：新增全字段相等的展示模型 `TocRuleUi`，`TocRules` 只对外暴露它。**注意：`TxtTocRule` 属逐字节搬运文件，此类问题一律在 Folio 侧解决，不要改搬运文件。**

**界面修复记录（续）：滚动时开关重播切换动画。** 现象是上下滑动规则列表时，部分刚进屏的开关会「滑一遍」到自己的位置。逐项测量排除了三条可能：`enable` 从不翻转（探针零命中）、开关收不到任何按压事件（注入 `MutableInteractionSource` 后零事件）、`hasContent` 为恒量——而 Material3 1.4.0 的 `Switch` 只可能由这三个输入驱动动画，于是按源码无法解释。**转折点来自使用者的一条观察：把规则全部开启或全部关闭后现象消失，状态混杂时才复现。** 这把方向从「框架通用缺陷」纠正为「逐行触发」：`Switch` 把滑块的动画状态存在 `Modifier.Node` 的普通字段（不是 `remember` 状态），LazyColumn 回收条目组合时，若新条目的开关状态与槽位前身相反，节点就带着旧位置，新条目一出现便从旧位置滑到新位置——把「上一条目被切换过」的动画重播了一遍。这同时解释了「有概率」（取决于槽位前身状态）与「只有部分行动」。修法是 `items(..., contentType = { it.enable })`：`contentType` 正是 Compose 用来划分复用池的 API，状态不同就不该共享复用池。此修复无可见取舍。

> 教训：`contentType` 只在「条目组合的复用安全性依赖某维度」时才想起来用，容易漏；而漏掉的症状是「滚动时偶发动画重播」，与数据无关、与触摸无关，从日志上完全看不见。

**设备验证补充（Android 16 / API 36 模拟器）**：ColorOS 会拦截 adb 安装，故改用官方模拟器
（`system-images;android-36;google_apis;x86_64`，AVD 名 `folio_api36`）做验收，安装与截图全自动。
- 验收原始场景：无「第N章」标记的日记体 TXT，目录由移植前的「整本一章」变为按字数兜底分章，可跳转。
- 规则预览在真机显示各规则章数；删除规则确认框正确代入规则名（修好前此处崩溃），执行后规则确实消失。

**与移植无关但同轮修掉的三个问题**：
1. `strings.xml` 的 `toc_rule_delete_confirm` 格式符被写坏（`%1$s` 只剩 `%1`，早前用 shell 传脚本时 `$s` 被 shell 吃掉），
   `String.format` 抛异常导致点删除即闪退；已修，并新增 `StringResourcesFormatTest` 全量校验格式符（aapt2 编译期拦不住）。
2. `LocalBook` 查源文件指纹时把 `SIZE` 与 `last_modified` 合并在一次查询，而并非所有 ContentProvider 都支持 `last_modified`
   （MediaStore 只有 `date_modified`），整条查询抛异常导致书打不开；改为分列查询并各自容错。
3. 选中的规则一条都不匹配时引擎返回空列表、书会打不开；加兜底退回自动择优。

**已知差异（需你裁决，未擅自处理）**：

1. **装饰符前缀标题的覆盖变窄。** 移植前的 `ChapterDetector` 正则在 legado 默认「目录」规则基础上加了 `(?:[☆★※✿＊*·•\-—_]+[、]?)?`，因此 `※第一章`、`·第一章`、`—第一章` 这类标题能被识别。移植后由规则库负责，其中启用规则「特殊符号 标题(单个)」只覆盖 `☆★✦✧`，「数字 分隔符 标题名称」要求前置数字——故 `※/✿/＊/·/•/—` 开头的标题会退化为「按字数分章」（仍可读、可跳转，但切不到真实章节边界）。保持原能力只需在规则库加一条：`^[ 　\t]{0,4}(?:[☆★※✿＊*·•\-—_]+[、]?)?(?:序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|第\s{0,4}[\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+?\s{0,4}(?:章|节(?!课)|卷|集(?![合和])|部(?![分赛游])|篇(?!张))).{0,30}$`。我未擅自加入，因为它会让 Folio 的默认规则集偏离 legado（择优打分可能与 legado 不同），也可能造成「已播种规则表的设备拿不到新规则」的补种问题——两者都属应与你商量的取舍。

2. ~~legado 的「规则预览」页尚未实现。~~ **已完成**：打开规则页时后台逐条试切、
   在规则名上方显示「N 章」与试算进度；试切不取正文、不落库，空正则条目按语义显示「按字数分章」。
   legado 侧另有独立的预览页与其开销循环，这里是功能等价的内嵌实现（同一处即可比较与选择）。

3. ~~`splitLongChapter` 尚未做成设置项。~~ **先做成设置项，后按使用者决定整个移除**：
   使用者的判断是触发条件太窄（只对「规则命中但命中段极长」的书生效）且没有反馈；
   同时该功能会在目录里留下人造的「原标题(1)(2)」与一个被置空的原文（legado 原样行为）。
   现 Folio 固定不拆（`TxtTocEngine.parse` 里 `splitLongChapterEnabled = false`），
   设置项/设置仓库/解析设置签名一并删除，页表缓存版本 bump 到 `ch6` 使旧缓存失效。
   引擎内那段拆分逻辑仍在逐字节搬运的 `TextFile.kt` 里原样保留、只是不被触发，恢复只需改一行。
