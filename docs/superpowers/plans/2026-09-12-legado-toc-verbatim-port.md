...
**界面修复记录（2026-09-12）**：开关失灵不是点击没送达——日志证实  触发且写入落库，但界面渲染探针在点击后再未触发。根因是移植来的  重写 / 只比较 uid=197108(Administrator) gid=197121 groups=197121（legado 原样，用于其列表去重），而 Compose 的  按结构相等判定状态变化，于是「只有 enable 变了」的新列表被判为与旧列表相等、更新被丢弃。修复落在 Folio 侧：新增全字段相等的展示模型 ， 只对外暴露它。**注意： 属逐字节搬运文件，此类问题一律在 Folio 侧解决，不要改搬运文件。**
...
2. **legado 的「规则预览」页尚未实现。** 规则管理（增删改、启停、恢复内置）与逐本规则选择已完成；legado 的预览页（预先算出每条规则能把当前书切成多少章，供选规则前比对）仍未做——它需要一个按规则反复试切的开销循环（legado 侧为 ），属独立改动。上面第 1 条的补救已可通过规则页「新建」完成。
