...
2. **legado 的「规则预览」页尚未实现。** 规则管理（增删改、启停、恢复内置）与逐本规则选择已完成；legado 的预览页（预先算出每条规则能把当前书切成多少章，供选规则前比对）仍未做——它需要一个按规则反复试切的开销循环（legado 侧为 `TxtTocRulePreviewViewModel`），属独立改动。上面第 1 条的补救已可通过规则页「新建」完成。

3. **`splitLongChapter` 尚未做成设置项。** 当前固定为 legado 的默认值 `true`；`readBook(..., splitLongChapter)` 参数已预留。
